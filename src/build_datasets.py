"""
Build train/test datasets for XGBoost, FastText, and Hierarchical BERT.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build split datasets for resume classification models."
    )
    parser.add_argument(
        "--input",
    ...
    parser.add_argument(
        ...
        type=int,
        default=8,
        help="Max chunks per resume for Hierarchical BERT. Default: 8",
    )
    parser.add_argument(
...
def clean_text(text: object) -> str:
    """Clean resume text while preserving punctuation useful for sentence splitting."""
    text = str(text)
    text = text.strip().strip('"')

    # Fix common mojibake observed in the dataset.
    text = text.replace("锛?", ",")
    text = text.replace("路", " ")
...
def mark_section_boundaries(text: str) -> str:
    """Add newlines around resume section titles."""
...
def split_sentences(text: str) -> list[str]:
    """Split text into resume-aware semantic units.

    Rules:
    - Section titles and newlines are strong boundaries.
    - . ? ! ; are sentence boundaries.
    - Comma is not a normal boundary. It is used later only as fallback for
      overly long units.
    """
    ...
    for part in parts:
        ...
        # Split after sentence-ending punctuation when followed by a likely new sentence.
...
def get_token_counter(tokenizer_name: str) -> Callable[[str], int]:
    """Return a tokenizer-based counter when transformers is available.

    If transformers/tokenizer is unavailable, fall back to a regex word counter.
    The fallback is enough for building chunk strings, while exact tensor
    creation should still be done inside the Hierarchical BERT Dataset.
    """
    try:
...
def split_long_unit(
    ...
    """Split an overly long unit, using comma only as a fallback."""
...
def split_by_words(text: str, max_tokens: int) -> list[str]:
    """Last-resort fallback for very long text without punctuation."""
    words = text.split()
    if not words:
        return []
    return [
...
def build_hbert_chunks(
    ...
    """Create sentence-aware chunks for Hierarchical BERT."""
...
def stratified_split(
    ...
    """Split data by label with a fixed test ratio."""
...
def main() -> None:
    ...
    df = read_input(input_path, args.text_col, args.label_col)
